...
Read `codecrib.local.md` in the plugin directory to get configuration:
...
- **project mode**: `codecrib-{project-name}`
  - Example: Working in `claude-crib` → collection: `codecrib-claude-crib`
- **shared mode**: `codecrib`
...
Read `codecrib.local.md` to get `vector_db` setting:
...
2. If collection doesn't exist, create with chroma_create_collection:
   ...
   - metadata: { "description": "CodeCrib knowledge base", "project": project_name }
```
...
Example: `claude-crib-plugins-codecrib-abc123`
...
```
Indexing to collection: codecrib-claude-crib
Mode: project

Indexing: 15/25 documents...
✓ Indexed: root.md
✓ Indexed: plugins-codecrib.md
...

Complete: 25 indexed, 0 skipped
Collection: codecrib-claude-crib (25 documents)
```
...
|--------|--------------|-------------|
| Collection | `codecrib-{project}` | `codecrib` |
